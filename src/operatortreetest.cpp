...
void OperatorTreeTest::constructor_ansNotFirstUseAndAdition()
{
	...
	OperatorTree tree(expression, 3, AngleTypeRadiant);
...
void OperatorTreeTest::calculateValue_sin360AndAngleTypeDegree_0()
{
	string expression("sin(360)");
	bool error;

	OperatorTree tree(expression, 0, AngleTypeDegree);

	CPPUNIT_ASSERT_DOUBLES_EQUAL(0, tree.calculateValue(error), 0.000001);
}

void OperatorTreeTest::calculateValue_sin360AndAngleTypeRadiant_correctResult()
{
	string expression("sin(360)");
	bool error;
	...
	CPPUNIT_ASSERT_DOUBLES_EQUAL(0.95891572, tree.calculateValue(error), 0.0001);
}